<template>
    <view class="reading full-screen" :style="{background: skin.pageBgColor}" @click="clickCenter">
        ...
        <view class="mask-top" :style="{top:isShowMask?0:-100+'rpx','background':skin.maskBgColor}">
            <view class="mask-title" :style="'color:' + skin.fontColor">{{realTimeInfo.chapter || chapterInfo.chapter}}
            </view>
        </view>
        <!-- 遮罩层下部分 -->
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            directoryBtn() {
                uni.navigateTo({
                    url: '/pages/reading/Directory?directory=' + encodeURIComponent(JSON.stringify(this.directory)) + '&currentChapterId=' + encodeURIComponent(JSON.stringify(this.realTimeInfo.chaptersId))
                });
            },
            //滑块设置字体间距或大小
            sliderChange(e, type) {
